...
CREATE PROC [dbo].[usp_Cpu_intensive]
AS
SELECT
er.session_id
,er.request_id
...
,er.start_time
,CONVERT(DECIMAL(21,3),er.total_elapsed_time / 1000.) AS [duration (ms)]
,er.cpu_time
,SUBSTRING(
qt.text,
...
AND qt.text NOT IN ('xp_userlock','sp_server_diagnostics')
AND er.database_id != 1
ORDER BY
er.cpu_time desc
